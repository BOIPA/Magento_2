...
namespace BOIPA\Payment\Helper;

use BOIPA\Payment\Helper\sdk\lib\Configurable;
use BOIPA\Payment\Helper\sdk\lib\Payments;
use BOIPA\Payment\Model\Config\Source\DisplayMode;
use Magento\Framework\App\Helper\AbstractHelper;
use BOIPA\Payment\Model\Config\Source\Environment;
/**
...
class Helper extends AbstractHelper
{
    ...
    {
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/boipa.log');
    ...
    public function getNotificationRoute($orderId)
    {
        $displayMode = $this->getConfigData('display_mode');
        if ($displayMode === DisplayMode::DISPLAY_MODE_REDIRECT) {
            return 'boipa/hosted/callback/orderid/' . $orderId;
        } else if ($displayMode === DisplayMode::DISPLAY_MODE_HOSTEDPAY) {
            return 'boipa/hosted/callback/orderid/' . $orderId;
        }else if ($displayMode === DisplayMode::DISPLAY_MODE_EMBEDDED) {
    ...
    public function getLandingPageOnReturnAfterRedirect($orderId)
    {
        $displayMode = $this->getConfigData('display_mode');
        if ($displayMode === DisplayMode::DISPLAY_MODE_REDIRECT) {
            return 'boipa/hosted/response/orderid/' . $orderId;
        } else if ($displayMode === DisplayMode::DISPLAY_MODE_HOSTEDPAY) {
            return 'boipa/hosted/response/orderid/' . $orderId;
        }else if ($displayMode === DisplayMode::DISPLAY_MODE_EMBEDDED) {
